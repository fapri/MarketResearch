# Matches dates and basis values for sinlge an entire folder
...
fullSet = data.frame()
dates = read_excel("Basis/Dates.xlsx")

cropType = dlg_message(message = "Yes for Corn, No for Soybeans", type = "yesno")[["res"]]

if (cropType == "yes") {
  files = list.files("Basis/CornData")
} else {
  files = list.files("Basis/SoybeanData")
}
...
for (filename in files) {
  ...
  if (cropType == "yes") {
    basis = read_csv(paste("Basis/CornData/", filename, sep = ""), skip = 1)
  } else {
    basis = read_csv(paste("Basis/SoybeanData/", filename, sep = ""), skip = 1)
  }
...
if (dlg_message(message = "Export data to a CSV?", type = "yesno")[["res"]] == "yes") {
  ...
  write.csv(fullSet, paste("Basis/", filename, ".csv", sep = ""), row.names = FALSE)
}
