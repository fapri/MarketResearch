# Cleans data from KState and puts it into format for plotting
# Weekly, yearly, and 3/5 year averages calculated
# Saved to RDS
...
# Load files
KStateFile = read.csv(choose.files(default = paste0(getwd(), "/Basis/*.csv"), caption = "Select Crop File:",
             multi = FALSE, filters = Filters,
             index = nrow(Filters)))

citiesCounties = read_excel("Basis/Cities and Counties.xlsx")
countyCenters = read_excel("Basis/County Centers.xlsx")
...
if (dlg_message(message = "Export kLocBasisMerge to a RDS?", type = "yesno")[["res"]] == "yes") {
  ...
  saveRDS(kLocBasisMerge, paste("Basis/", filename1, ".rds", sep = ""))
}
...
if (dlg_message(message = "Export weeklyAverageList to a RDS?", type = "yesno")[["res"]] == "yes") {
  ...
  saveRDS(weeklyAverageList, paste("Basis/", filename1, ".rds", sep = ""))
}
